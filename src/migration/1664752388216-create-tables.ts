...
export class createTables1664752388216 implements MigrationInterface {
    name = 'createTables1664752388216'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "tenants" ("id" SERIAL NOT NULL, "tenant_name" character varying NOT NULL, "password" character varying NOT NULL, "season" integer NOT NULL DEFAULT '1', "slack" character varying NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_5cbe54e309efc7de9cf098223c8" UNIQUE ("tenant_name"), CONSTRAINT "PK_53be67a04681c66b87ee27c9321" PRIMARY KEY ("id")); COMMENT ON COLUMN "tenants"."id" IS 'テナントID'; COMMENT ON COLUMN "tenants"."tenant_name" IS 'テナント名'; COMMENT ON COLUMN "tenants"."password" IS 'テナントパスワード'; COMMENT ON COLUMN "tenants"."season" IS 'シーズンID'; COMMENT ON COLUMN "tenants"."slack" IS 'slack連携ID'; COMMENT ON COLUMN "tenants"."created_at" IS '登録日時'; COMMENT ON COLUMN "tenants"."updated_at" IS '更新日時'`);
        ...
        await queryRunner.query(`CREATE TABLE "teams" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "penalty" integer NOT NULL DEFAULT '0', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "tenantId" integer, CONSTRAINT "PK_7e5523774a38b08a6236d322403" PRIMARY KEY ("id")); COMMENT ON COLUMN "teams"."id" IS 'チームID'; COMMENT ON COLUMN "teams"."name" IS 'チーム名'; COMMENT ON COLUMN "teams"."penalty" IS 'ペナルティポイント'; COMMENT ON COLUMN "teams"."created_at" IS '登録日時'; COMMENT ON COLUMN "teams"."updated_at" IS '更新日時'; COMMENT ON COLUMN "teams"."tenantId" IS 'テナントID'`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_1509c0ea804268d873721dd043" ON "teams" ("tenantId", "name") `);
        await queryRunner.query(`CREATE TABLE "users" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "password" character varying NOT NULL, "point" integer NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "tenantId" integer, "roleId" integer, "teamId" integer, CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id")); COMMENT ON COLUMN "users"."created_at" IS '登録日時'; COMMENT ON COLUMN "users"."updated_at" IS '更新日時'; COMMENT ON COLUMN "users"."tenantId" IS 'テナントID'; COMMENT ON COLUMN "users"."roleId" IS 'ロールID'; COMMENT ON COLUMN "users"."teamId" IS 'チームID'`);
        ...
        await queryRunner.query(`ALTER TABLE "teams" ADD CONSTRAINT "FK_e70292ca0e2e6126aa4f09a42d5" FOREIGN KEY ("tenantId") REFERENCES "tenants"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
