...
export class createTables1666221757634 implements MigrationInterface {
    name = 'createTables1666221757634'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "roles" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "created_at" TIMESTAMP(0) NOT NULL DEFAULT now(), "updated_at" TIMESTAMP(0) NOT NULL DEFAULT now(), CONSTRAINT "PK_c1433d71a4838793a49dcad46ab" PRIMARY KEY ("id")); COMMENT ON COLUMN "roles"."id" IS 'ロールID'; COMMENT ON COLUMN "roles"."name" IS 'ロール名'; COMMENT ON COLUMN "roles"."created_at" IS '登録日時'; COMMENT ON COLUMN "roles"."updated_at" IS '更新日時'`);
